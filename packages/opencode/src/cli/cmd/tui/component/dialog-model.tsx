...
export function useConnected() {
  const sync = useSync()
  return createMemo(() =>
    sync.data.provider.some((x) => x.id !== "corethink" || Object.values(x.models).some((y) => y.cost?.input !== 0)),
  )
}
...
export function DialogModel(props: { providerID?: string }) {
  ...
  const options = createMemo(() => {
      ...
      return [
        {
          ...
          category: "Favorites",
          disabled: provider.id === "corethink" && model.id.includes("-nano"),
          footer: model.cost?.input === 0 && provider.id === "corethink" ? "Free" : undefined,
          onSelect: () => {
            dialog.clear()
            local.model.set(
              {
    ...
    const providerOptions = pipe(
      sync.data.provider,
      sortBy(
        (provider) => provider.id !== "corethink",
        ...
        pipe(
          ...
          map(([model, info]) => {
            ...
            return {
              ...
              disabled: provider.id === "corethink" && model.includes("-nano"),
              footer: info.cost?.input === 0 && provider.id === "corethink" ? "Free" : undefined,
              onSelect() {
                dialog.clear()
                local.model.set(
                  {
